...
测试（**本机实跑，非静态计数**）：pytest **305/305**（v0.2c 254 + v0.3 MCP 读 13 + 写/webhook 38）· vitest **36/36**（v0.5 ⌘K 命令面板 +8）· Rust **17/17**（keystore_smoke 8 + llm_config_smoke 9）· Playwright E2E **7/7**（命令面板 +2）· `cargo check --all-targets` + `npm run build` 干净。复核：`cd python && uv run pytest -v` / `npm test` / `cargo test` / `npm run test:e2e`。
