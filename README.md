...
测试（**本机实跑，非静态计数**）：pytest **323/323**（v0.2c 254 + v0.3 MCP 读 13 + 写/webhook 38 + v0.5 标签 18）· vitest **42/42**（v0.5 ⌘K 命令面板 +8 / 标签 +6）· Rust **17/17**（keystore_smoke 8 + llm_config_smoke 9）· Playwright E2E **7/7**（命令面板 +2）· `cargo check --all-targets` + `npm run build` 干净。复核：`cd python && uv run pytest -v` / `npm test` / `cargo test` / `npm run test:e2e`。
